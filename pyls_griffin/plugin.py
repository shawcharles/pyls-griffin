...
# Local imports
from pyls_griffin.utils import RegexEvaluator
...
def pylsp_document_symbols(config: Config,
    ...
    settings = config.plugin_settings('pyls_griffin')
    ...
    griffin_symbols = cells + blocks
    griffin_symbols = sorted(
        griffin_symbols, key=lambda x: x['location']['range']['start']['line'])
    return griffin_symbols
